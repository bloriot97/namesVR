import {
    Engine, Scene, ArcRotateCamera, HemisphericLight, Vector3, MeshBuilder, Mesh, MultiMaterial, StandardMaterial, Color3, SceneOptimizerOptions, HardwareScalingOptimization, SceneOptimizer,
} from "babylonjs"
...
const genScene = (nameData) => {
    ...
    const createDefaultEngine = function () { return new Engine(canvas, true, { preserveDrawingBuffer: true, stencil: true }) }
    const createScene = function () {
        ...
        if (colorMode === "GENDER") {
            const material1 = new StandardMaterial("sphere material", scene)
            material1.diffuseColor = new Color3(0, 0, 1)
            material1.freeze()
            multimat.subMaterials.push(material1)

            const material2 = new StandardMaterial("sphere material", scene)
        ...
        for (let i = 0; i < nameData.length; i++) {
            ...
            const sphere = MeshBuilder.CreateBox("sphere", { size })
        ...
        const newMesh = Mesh.MergeMeshes(boxes, true, true, undefined, true)
        ...
        const camera = new ArcRotateCamera("camera", -Math.PI / 2, Math.PI / 4, 0.1, new Vector3(0, 3, 0), scene)
        console.log(camera)
        ...
        const plane = MeshBuilder.CreatePlane("plane", { size: 1, sideOrientation: Mesh.BACKSIDE }, scene)
        ...
        // Enable VR
        const vrHelper = scene.createDefaultVRExperience({ createDeviceOrientationCamera: false, useMultiview: true })
...
axios.get("/public/data_3D_sample.json")
    .then((response) => {
